...
class SidebarPartialView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['recent_posts'] = Post.objects.order_by('-date_published')[:8]
        context['categories'] = Category.objects.order_by('-id')
        context['tags'] = ['جی تی ای', 'فیلم', 'بازی', 'سریال کره ای ', 'آیفون', 'بررسی گوشی']
        return context
...
def all_newposts(request):
    ...
    paginator = Paginator(newest_game_posts, 4)
    page_list = paginator.get_page(page_number)
    return render(request, 'blog/blog_list.html', {'posts': page_list})
